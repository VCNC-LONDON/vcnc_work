WITH dummy_time_table AS (
      SELECT
        DATE(time, 'Asia/Seoul') AS date_kr,
        DATETIME(time, 'Asia/Seoul') AS time_interval_kr,
      FROM
        UNNEST(GENERATE_TIMESTAMP_ARRAY(TIMESTAMP(DATE_SUB("{target_date}", INTERVAL 1 DAY), 'Asia/Seoul'),
                                        TIMESTAMP(DATE_ADD("{target_date}", INTERVAL 1 DAY), 'Asia/Seoul'), INTERVAL 30 MINUTE)) time
      )
    ...
    , demand_base AS (
      SELECT
        dummy_time_table.date_kr,
        dummy_time_table.time_interval_kr,
        IFNULL(X, 0) AS X,
        IFNULL(L, 0) AS L,
        IFNULL(P, 0) AS P,
        IFNULL(N, 0) AS N,
        ...
        IFNULL(TL, 0) AS TL,
        IFNULL(TP, 0) AS TP,
        IFNULL(ROUND(SAFE_DIVIDE(X_surge, TX)), 100) AS X_surge,
        IFNULL(ROUND(SAFE_DIVIDE(L_surge, TL)), 100) AS L_surge,
        IFNULL(ROUND(SAFE_DIVIDE(P_surge, TP)), 100) AS P_surge,
      FROM
        dummy_time_table LEFT JOIN _demand_base
        USING(time_interval_kr)
      )
    ...
    , supply_base AS (
      ...
      FROM
        dummy_time_table LEFT JOIN _supply_base
        USING(time_interval_kr)
      )
