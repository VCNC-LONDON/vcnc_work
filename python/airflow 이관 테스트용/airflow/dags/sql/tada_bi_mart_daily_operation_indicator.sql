WITH store_aggregated AS (
  SELECT
    ...
    #매출
    sum(ifnull(receipt_drive_fee, 0)) + sum(ifnull(receipt_cancellation_fee, 0)) + sum(ifnull(receipt_tollgate_fee, 0)) + sum(ifnull(credit_withdrawal_amount, 0)) AS revenue_no_credit,
    sum(ifnull(receipt_total, 0)) + sum(ifnull(credit_withdrawal_amount, 0)) AS net_revenue,
    (sum(ifnull(receipt_drive_fee, 0)) + sum(ifnull(receipt_cancellation_fee, 0)) + sum(ifnull(receipt_tollgate_fee, 0)) + sum(ifnull(credit_withdrawal_amount, 0))) / 1.1 AS revenue_no_credit_without_vat,
    (sum(ifnull(receipt_total, 0)) + sum(ifnull(credit_withdrawal_amount, 0))) / 1.1 AS net_revenue_without_vat
...
finalised AS (
  SELECT
    ...
    a.driver_type,
    b.valid_estimation_view_count,
    a.* EXCEPT(date_kr, origin_region, driver_type, credit_withdrawal_amount_without_vat, net_revenue_without_vat, revenue_no_credit, net_revenue, revenue_no_credit_without_vat),
    a.revenue_no_credit_without_vat + a.credit_withdrawal_amount_without_vat AS revenue_without_vat,
    a.net_revenue_without_vat,
    1 - (net_revenue_without_vat / (a.revenue_no_credit_without_vat + a.credit_withdrawal_amount_without_vat)) AS discount_rate,
    c.picked_up_hour,
