...
server_log_type = "USER_UPDATE_PREFERENCES"
...
SELECT 
  caller.gaid AS gaid,
  caller.idfa AS idfa,
  caller.ipAddress AS ip_address,
  caller.userAgent AS user_agent,
  caller.userId AS user_id,
  DATETIME(TIMESTAMP_MILLIS(timeMs), 'Asia/Seoul') AS datetime_kr,
  type,
  uuid,
  userPreferencesLog.beforePreferences,
  userPreferencesLog.afterPreferences,
  date_kr
...
schema_description = [
    {"name": "gaid", "description": "Andorid Ad ID"},
    {"name": "idfa", "description": "IOS Ad ID"},
    {"name": "ip_address", "description": "IP 주소"},
    {"name": "user_agent", "description": "user_agent"},
    {"name": "user_id", "description": "user_id"},
    {"name": "datetime_kr", "description": "실행한 DATETIME"},
    {"name": "type", "description": f"서버 로그 타입({server_log_type})"},
    {"name": "uuid", "description": "UUID"},
    {"name": "beforePreferences", "description": "직전 설정"},
    {"name": "afterPreferences", "description": "이후 설정"},
