...
WHERE
  _TABLE_SUFFIX BETWEEN FORMAT_DATE("%Y%m%d", DATE_SUB("{execute_date}", INTERVAL 1 day)) AND FORMAT_DATE("%Y%m%d", "{execute_date}")
  AND DATE(DATETIME(protopayload_auditlog.servicedata_v1_bigquery.jobCompletedEvent.job.jobStatistics.startTime, "Asia/Seoul")) = "{execute_date}"
"""
