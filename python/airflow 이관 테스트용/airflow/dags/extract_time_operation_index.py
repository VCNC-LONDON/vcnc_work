...
with models.DAG(
    dag_id="Extract-time_operation_index",
    description="time_operation_index",
    ...
    for i in range(0, 2):
        extract_time_operation_task = BigQueryOperator(
            dag=dag,
            bigquery_conn_id="google_cloud_for_tada",
            task_id=f"extract_time_operation_index_{rule[i]}",
            sql=utils.read_sql(
                os.path.join(os.environ["DAGS_FOLDER"], "sql/time_operation_index.sql")
            ).format(target_date=date_suffix[i]),
            use_legacy_sql=False,
            destination_dataset_table=f"kr-co-vcnc-tada.tada_store.time_operation_index${date_suffix_formatted[i]}",
            write_disposition="WRITE_TRUNCATE",
            time_partitioning={"type": "DAY", "field": "date_kr"},
            on_failure_callback=task_fail_slack_alert,
        )

        extract_time_operation_task >> combine_task

    add_description_task = PythonOperator(
        task_id="description_task",
        python_callable=add_description.update_schema_description,
        op_kwargs={
            "table_fullname": "kr-co-vcnc-tada.tada_store.time_operation_index",
